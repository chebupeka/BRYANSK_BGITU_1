...
`docx_generator.py` применяет порядок блоков и `placement` реквизитов из типа и оформление из шаблона. Соседние реквизиты с одинаковым `placement` объединяются: дата и номер — в одну строку, должность и расшифровка подписи — в строку с правой табуляцией. Генератор не обращается к модели. Вид документа, префиксы вроде «№ » и подписи полей — статическая структура; значения приходят от пользователя или заменяются меткой. Пробелы в «№ 214» и «30 000» заменяются неразрывными, буквы и цифры не меняются. Стили используют шрифт шаблона без шрифтов темы Word, язык документа — русский. Файл состоит из текстовых абзацев, не из изображений и таблиц. В верхнем колонтитуле со второй страницы — номер страницы.
