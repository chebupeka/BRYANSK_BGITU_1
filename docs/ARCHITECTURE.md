...
3. `extract_facts` сводит даты, числа и имена к сравнимому виду: `25.09.2026` и «25 сентября 2026» равны, «30 000» равно «30000», нумерация списка фактом не считается. Сокращение имени до инициалов разрешено, раскрытие инициалов в полное имя — нет.
...
5. Ответы пользователя приоритетнее значений модели и никогда ими не перезаписываются.

Чего в адаптере нет: кэша по хэшу черновика, типа и ответов пользователя (оформление в ключ не включать); словесных чисел и падежей при сверке — «тридцать тысяч» и «30 000» считаются разными сведениями и дают предупреждение; потоковой выдачи и оценки качества на длинных документах.
