...
`docx_generator.py` применяет порядок блоков и `placement` реквизитов из типа и оформление из шаблона. Соседние реквизиты с одинаковым `placement` объединяются: дата и номер — в одну строку, должность и расшифровка — в подпись. Шаблон решает, куда попадают организация (верхний колонтитул или начало страницы) и адресат (блок или таблица «Кому / От кого»), что стоит в нижнем колонтитуле (номер страницы или название документа с датой) и как выровнена подпись. Генератор не обращается к модели. Вид документа, префиксы вроде «№ » и подписи полей — статическая структура; значения приходят от пользователя или заменяются меткой и переносятся в файл символ в символ. Стили используют шрифт шаблона без шрифтов темы Word, язык документа — русский. Файл состоит из текстовых абзацев и одной таблицы адресата в «Современном регламентном», без изображений.
