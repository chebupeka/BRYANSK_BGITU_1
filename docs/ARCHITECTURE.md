...
5. Реквизит с `summary: true` в конфигурации типа (сейчас это только тема) модель формулирует по смыслу черновика; остальные поля она заполняет лишь там, где они названы прямо.
6. Ответы пользователя приоритетнее значений модели и никогда ими не перезаписываются.

Чего в адаптере нет: кэша по хэшу черновика, типа и ответов пользователя (оформление в ключ не включать); потоковой выдачи; оценки качества на длинных документах. Сверка остаётся приблизительной: она ловит числа, даты и фамилии, но не проверяет, что смысл условия сохранён.
