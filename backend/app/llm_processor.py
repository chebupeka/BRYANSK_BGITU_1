"""Содержательная обработка черновика моделью: промпт, правки и сверка фактов.

Модуль принадлежит роли 2 и работает по OpenAI-совместимому API
(`/v1/chat/completions`), который отдаёт Ollama. Ответ модели проверяется схемой и
независимой сверкой фактов: сведения, которых нет в черновике или в ответах
пользователя, не попадают в документ молча.

К границе приложения модуль подключает `LLMTextProcessor` в конце файла: он приводит
результат к общему `app.processing.ProcessorResult`, а отказы — к единому контракту
ошибок по таблице `MODEL_FAILURES`.
"""
...
EXAMPLE_DRAFT = (
    "Прошу выдилить деньги на ремонт принтера, а то печатать нечем. "
    "Сообщаю что смета готова: нужно 12 000 рублей, желательно до 14.03.2026, "
...
EXAMPLE_REPLY = (
    ...
    'из строя.", "Сообщаю, что смета подготовлена. Стоимость ремонта составляет 12 000 рублей. '
    'Работы необходимо выполнить до 14.03.2026 при условии наличия средств."]}'
)
RETRY_HINT = (
...
# Причина отказа → HTTP-статус и признак повтора в общем контракте ошибок роли 1.
MODEL_FAILURES: dict[str, tuple[int, bool]] = {
    "llm_timeout": (504, True),
    "llm_connection_error": (503, True),
    "llm_model_missing": (503, False),
    "llm_upstream_error": (502, True),
    "llm_invalid_output": (502, False),
}


class ModelUnavailable(Exception):
    """Отказ обработки. Сообщение безопасно для клиента: ответа провайдера в нём нет."""
...
@dataclass
class ModelResult:
    """Что обработчик передаёт адаптеру: содержание, перечень правок и предупреждения."""
...
class LLMProcessor:
    ...
    def close(self) -> None:
        """Приложение закрывает то, что создало само: подменённый клиент — забота теста."""
        if self._owns_client:
            self._client.close()

    def process(self, request: ProcessRequest, doc_type: DocumentType) -> ModelResult:
    ...
    def _ask(self, messages: list[dict[str, str]]) -> str:
        ...
        try:
            ...
            raise ModelUnavailable(
                f"Сервер модели ответил ошибкой {error.response.status_code}. "
...
class LLMTextProcessor:
    """Адаптер обработчика роли 2 к границе `TextProcessor` роли 1.

    Реализация выше работает с типом документа и своим результатом, а приложение ждёт
    `process(request)`, общий `ProcessorResult` и `close()`. Отказы переводятся в единый
    контракт ошибок: код, статус и признак повтора выбираются по причине, а текст ответа
    провайдера клиенту не уходит.
    """

    def __init__(self, processor: LLMProcessor) -> None:
    ...
    def process(self, request: ProcessRequest) -> ProcessorResult:
        if not self._processor.model:
            raise ProcessorUnavailable(
                "Укажите LLM_MODEL для подключения модели.",
                code="llm_not_configured", status=503, retryable=False,
            )
        doc_type = document_types()[request.doc_type]
        try:
            result = self._processor.process(request, doc_type)
        except ModelUnavailable as error:
            status, retryable = MODEL_FAILURES.get(error.code, (503, True))
            raise ProcessorUnavailable(
                str(error), code=error.code, status=status, retryable=retryable
            ) from None
