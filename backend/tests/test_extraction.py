...
@pytest.mark.parametrize(("line", "expected"), [
    ("Приложение на 2 листах", "на 2 листах"),
    ("Приложение на 1 листе", "на 1 листе"),
    ("Приложения на 5 л. в 1 экз.", "на 5 л. в 1 экз"),
    ("ПРИЛОЖЕНИЕ 3 листа", "3 листа"),
    ("Приложение: смета на 2 л. в 1 экз.", "смета на 2 л. в 1 экз"),
    ("Приложение — договор поставки", "договор поставки"),
])
@pytest.mark.parametrize("type_id", ["service_memo", "report_memo", "letter"])
def test_attachment_without_separator_is_only_a_sheet_count(line, expected, type_id):
    """Без разделителя берётся только объём, описание — через двоеточие или тире."""
    for placement in ("before-text", "closing"):
        draft = PLACEMENTS[placement].format(line=line)
        doc_type = document_types()[type_id]

        assert suggest_requisites(draft, doc_type)["attachment"] == expected, placement
...
PROSE_LINES = [
    ...
    "Приложение не открывается на телефоне",
    "Приложение к письму потерялось",
    "Приложения нет",
    "Приложения к договору пока нет",
    "ПРИЛОЖЕНИЕ НЕ ОТКРЫВАЕТСЯ",
    "Приложение на телефоне обновилось",
    "Приложение на 2 дня задержали",
    "Приложение к письму на 2 листах потерялось",
    "Приложение 2 не открывается",
    "Приложение 1",
    "Приложение № 1 потерялось",
]
