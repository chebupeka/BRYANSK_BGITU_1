...
from app.processing import ProcessorUnavailable, get_processor
...
def prepared(processor, requisites=None, doc_type="service_memo", draft=DRAFT):
    app = create_app(
        Settings(text_processor="llm", llm_model="test-model"),
        processor=LLMTextProcessor(processor),
    )
...
def test_empty_body_from_model_is_refused():
    ...
    assert prepared(processor).status_code == 502
    assert len(sent) == 2


def test_llm_mode_requires_model_name():
    processor = get_processor(Settings(text_processor="llm", llm_model=""))
    request = ProcessRequest(doc_type="service_memo", draft=DRAFT, requisites={})
    with pytest.raises(ProcessorUnavailable, match="LLM_MODEL"):
