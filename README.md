...
scripts/                check_docker.py, check_llm.py, export_contracts.py, qa.py, qa_report.py
...
1. Без модели, в [extraction.py](backend/app/extraction.py): на шаге «Тип и реквизиты»
   ...
   в приложение (без разделителя — только объём в листах; описание приложения
   пишется через двоеточие). Найденные строки убираются из основного текста до запроса к модели.
...
- Реквизиты из черновика без модели берутся только из отдельных строк с меткой. Метка
  ...
  дата, номер и подписант, и только когда значение нельзя спутать с фразой:
  «Дата 12.03.2025», «Номер 47-СЗ», «Подпись Петров П.П.». Тема и заголовок без разделителя
  («Тема О закупке техники») не разбираются и остаются в тексте. Остальное, в том числе
  реквизит в конце абзаца («…в срок. Подпись Петров.»), остаётся в форме для ввода.
