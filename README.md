...
стилю. Сервис сверяет её ответ с черновиком и не пропускает в документ выдуманные даты,
суммы и имена. Оформление файла задают YAML-шаблоны, а не модель. Поддерживаются четыре
...
- Реквизиты из черновика без модели берутся только из строк с двоеточием или тире
  («Дата: 12.03.2025»). Строку «Дата 12.03.2025» без разделителя разбор не узнаёт, и поле
  остаётся в форме для ввода.
